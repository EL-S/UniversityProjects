...
class DemoWindow < Gosu::Window
  ...
  def draw
    ...
    Gosu.draw_rect(45, 45, 110, 60, @border, ZOrder::MIDDLE, mode=:default)
    # Draw the button
 ...
 # This still needs to be fixed!
  ...
  def update
    if mouse_over_button(mouse_x, mouse_y)
      @border = Gosu::Color::BLACK
    else
      if @background == Gosu::Color::YELLOW
        @border = Gosu::Color::YELLOW
      else
        @border = Gosu::Color::WHITE
      end
    end
  ...
  def button_down(id)
    ...
    when Gosu::MsLeft
      if mouse_over_button(mouse_x, mouse_y)
        if @background == Gosu::Color::YELLOW
          @background = Gosu::Color::WHITE
        elsif @background == Gosu::Color::WHITE
