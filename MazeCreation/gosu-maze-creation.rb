  ...
  def search(cell_x, cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
      ...
      if cell_obj.south != nil
        if cell_obj.south.vacant and !@columns[cell_x][cell_y+1].visited
          south_path = [cell_x,cell_y+1]
        end
      end
      if cell_obj.west != nil
        if cell_obj.west.vacant and !@columns[cell_x-1][cell_y].visited
      ...
      if (path != nil)
        ...
        if next_path == [nil]
          ...
          [[cell_x,cell_y]].concat(next_path)
        end
      else
        if (ARGV.length > 0) # debug
